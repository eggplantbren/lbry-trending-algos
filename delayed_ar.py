"""
This is a modified version of ar.py that adds a delay to the
effect of large, sudden LBC changes, to prevent whales from doing
the six-hour shuffle. Time delays (in blocks) are determined by this line:

    delay = min(int(math.sqrt(total_amount + 1E-8)), 1000)

where total_amount is in LBC.

It also has symmetric support/abandon behaviour (abandons have the
same effect as time-reversed negative supports).

It is currently running on my wallet server.
"""
